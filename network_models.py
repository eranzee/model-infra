...
class TransResNet18(nn.Module):
    def __init__(self, num_classes):
        super(TransResNet18, self).__init__()
        self.model_index = 0
        ...
        self.fc_rot0 = nn.Linear(512, num_classes)
        self.fc_rot90 = nn.Linear(512, num_classes)
        self.fc_rot180 = nn.Linear(512, num_classes)
        self.fc_rot270 = nn.Linear(512, num_classes)
    ...
    def forward(self, x, rot_type=0):
        ...
        if rot_type == 0:
            return self.fc_rot0(x)
        elif rot_type == 1:
            return self.fc_rot90(x)
        elif rot_type == 2:
            return self.fc_rot180(x)
        elif rot_type == 3:
            return self.fc_rot270(x)

        raise Exception("No such head!")


class RotResNet18(nn.Module):
    def __init__(self, num_classes):
        # Reset
        super(RotResNet18, self).__init__()
        self.model_index = 0
        self.resnet18 = models.resnet18()
...
class SimCLRResNet18(nn.Module):
    def __init__(self, num_classes):
        ...
        # MLP
        self.fc_mlp = nn.Linear(512, 512)
        self.relu_mlp = F.relu

    def forward(self, x, simclr_task=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
